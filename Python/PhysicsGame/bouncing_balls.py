...
class BouncyBalls(object):
    ...
    def run(self) -> None:
        ...
        while self._running:
            ...
            mouse_pos = pymunk.pygame_util.get_mouse_pos(self._screen)

            shape = self._space.point_query_nearest(
                mouse_pos, float("inf"), pymunk.ShapeFilter()
            ).shape
            if shape is not None and isinstance(shape, pymunk.Circle):
                r = shape.radius + 4
                p = pymunk.pygame_util.to_pygame(shape.body.position, self._screen)
                pygame.draw.circle(self._screen, pygame.Color("red"), p, int(r), 2)
