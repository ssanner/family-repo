...
def setup_level(space, player_bodies):

    # Remove balls and bricks
    for s in space.shapes[:]:
        if s.body.body_type == pymunk.Body.DYNAMIC and s.body not in player_bodies:
            space.remove(s.body, s)

    # Spawn a ball for the player to have something to play with
    spawn_ball(
        space, player_bodies[0].position + (0, 40), random.choice([(1, 10), (-1, 10)])
    )
...
def main():
    ...
    ### Player ship right
    player_body_r = pymunk.Body(500, float("inf"))
    player_body_r.position = 500, 100

    player_shape_r = pymunk.Segment(player_body_r, (-50, 0), (50, 0), 8)
    player_shape_r.color = pygame.Color("blue")
    player_shape_r.elasticity = 1.0
    player_shape_r.collision_type = collision_types["player"]
    ...
    space.add(player_body_r, player_shape_r)#, move_joint)
    global state
    # Start game
    setup_level(space, [player_body, player_body_r])
            ...
            try:
                ...
                if pose_results.pose_landmarks is not None:
                    ...
                    for idx, res in enumerate(pose_results.pose_landmarks.landmark):
                        if idx == 17: # left hand
                            cx, cy = int(width - res.x*width), int(res.y*height) # TODO
                            pygame.draw.circle(screen, (0,0,255), (cx, cy), 5, 1) #(r, g, b) is color, (x, y) is center, R is radius and w is the thickness of the circle border.
                            player_body.position = (cx, height - cy)
                        if idx == 20: # right hand
                            cx, cy = int(width - res.x*width), int(res.y*height) # TODO
                            pygame.draw.circle(screen, (255,0,0), (cx, cy), 5, 1) #(r, g, b) is color, (x, y) is center, R is radius and w is the thickness of the circle border.
                            player_body_r.position = (cx, height - cy)
